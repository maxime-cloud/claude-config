...
OAuth token read from `~/.claude/.credentials.json` (Linux/cross-platform):

- **File**: `~/.claude/.credentials.json`
- **Format**: `{ "claudeAiOauth": { "accessToken": "sk-ant-oat01-..." } }`
- **Token type**: `sk-ant-oat01-...` (OAuth token, not API key)

> macOS alternative: token can also be read from Keychain via `security find-generic-password -s "Claude Code-credentials" -w`
...
- Requires `~/.claude/.credentials.json` with a valid OAuth token (written by Claude Code on login)
