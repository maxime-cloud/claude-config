...
export function section(
	...
	if (options?.enabledPath) {
		const enabledPath = options.enabledPath;
		return {
			path,
			label,
			type: "section",
			getValue: (c) => getNestedValue(c, enabledPath) as boolean,
			toggle: (c) => {
				const current = getNestedValue(c, enabledPath) as boolean;
				setNestedValue(c, enabledPath, !current);
			},
		};
	}
